...
export function DashboardFinanzas() {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      turnos.forEach(t => {
        ...
        porDia[dia].ingreso += t.precio??0; porDia[dia].turnos += 1
      })
      ...
      turnos.forEach(t => {
        ...
        porServ[nombre].cantidad++; porServ[nombre].ingreso += t.precio??0
      })
      const por_servicio = Object.values(porServ).sort((a,b)=>b.ingreso-a.ingreso)
      setData({ ingreso_total, turnos_completados:turnos.length, clientes_unicos,
        metodo_efectivo, metodo_mp, metodo_transferencia,
        por_dia: Object.values(porDia), por_servicio, servicio_top: por_servicio[0]?.nombre??'--' })
      setLoading(false)
    }
    fetch()
  }, [periodo])
  ...
  const exportarCSV = () => {
    if (!data) return
    const filas = [['Servicio','Cantidad','Ingreso'],...data.por_servicio.map(s=>[s.nombre,s.cantidad,s.ingreso]),[],['Metodo','Monto'],['Efectivo',data.metodo_efectivo],['MercadoPago',data.metodo_mp],['Transferencia',data.metodo_transferencia]]
    const blob = new Blob([filas.map(f=>f.join(',')).join('\n')],{type:'text/csv'})
    const a = document.createElement('a'); a.href=URL.createObjectURL(blob)
    a.download=hebra--.csv; a.click()
  }

  if (loading) return <div className="space-y-4 animate-pulse">{[...Array(4)].map((_,i)=><div key={i} className="h-20 bg-white/5 rounded-xl"/>)}</div>
  if (!data) return (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-white text-xl font-bold">Finanzas</h2><p className="text-white/40 text-sm">Tu resumen financiero</p></div>
        <div className="flex gap-1 bg-white/5 rounded-lg p-0.5">
          {(['hoy','semana','mes'] as Periodo[]).map(p=>(
            <button key={p} onClick={()=>setPeriodo(p)} className={px-3 py-1.5 rounded-md text-xs font-medium transition-all }>
      ...
      <div className="bg-gradient-to-br from-green-500/10 to-green-500/0 border border-green-500/20 rounded-2xl p-6">
        ...
        <p className="text-white text-4xl font-bold mt-1"></p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          {icon:DollarSign,label:'Promedio turno',value:$,c:'text-purple-400 bg-purple-500/10'},
        ].map(({icon:Icon,label,value,c})=>(
          <div key={label} className="bg-white/3 border border-white/5 rounded-xl p-4">
            <div className={w-8 h-8 rounded-lg  flex items-center justify-center mb-2}><Icon size={16}/></div>
          ...
          <div className="flex items-end gap-2 h-32">
            {data.por_dia.map((d,i)=>(
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                <div className="w-full bg-green-500/60 rounded-t-md hover:bg-green-500 transition-colors" style={{height:${(d.ingreso/maxIngreso)*80}%,minHeight:'4px'}}/>
          ...
          {[
            ...
            <div key={m.label}>
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2"><m.icon size={14} className="text-white/40"/><span className="text-white/70 text-xs">{m.label}</span></div>
                <span className="text-white text-xs font-medium"> ({pct(m.monto)}%)</span>
              </div>
              <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div className={h-full  rounded-full transition-all duration-500} style={{width:${pct(m.monto)}%}}/>
              </div>
            </div>
          ))}
        </div>
      </div>
      <button onClick={exportarCSV} className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white/60 py-3 rounded-xl transition-colors text-sm">
